...
namespace Kitsune::Details
{
    ...
    // Make sure that the expression `expr` is always true. If it is not true, then the
    // specified message is logged, and the application will break if a debugger is connected.
    #define KITSUNE_ASSERT KITSUNE_ALWAYS_ASSERT_

    // Same thing as `KITSUNE_ASSERT`, but when building in Production mode, this macro
    // will still run the expression passed in. Use this instead of `KITSUNE_ASSERT` in
    // cases where the expression has an impact on the program's control flow.
