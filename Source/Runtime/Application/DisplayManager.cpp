...
namespace Kitsune
{
        ...
        const DisplayManagerSpecifications& specs)
    {
        ...
        if (specs.Headless)
        {
            ...
            s_Instance = CreateNullDisplayManager_(specs);
        }
        else
        {
#if defined(KITSUNE_OS_WINDOWS)
            s_Instance = Memory::New<WindowsDisplayManager>(
                L"Kitsune Window", specs);
#else
            KITSUNE_ENGINE_ERROR(
                DisplayManager,
                "Could not find an implementation for DisplayManager. "
                "Using the NULL implementation instead.");
    ...
    void DisplayManager::Shutdown()
    {
        ...
        {
            throw LogicException(
                "DisplayManager::Shutdown() called when the display "
    ...
    DisplayManager* DisplayManager::CreateNullDisplayManager_(
        const DisplayManagerSpecifications& specs)
    {
        ...
        return Memory::New<NullDisplayManager>(Move(screen), specs);
    }
}
